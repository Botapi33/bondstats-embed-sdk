...
(() => {
  ...
  const DEFAULT_HOME = 'https://www.bondstats.org/';
  ...
  function parseFeed(raw) {
    ...
    if (raw && raw.countries && typeof raw.countries === 'object') {
      for (const [key, row] of Object.entries(raw.countries)) {
        ...
        let change = number(row.change);
        if (!Number.isFinite(change) && Number.isFinite(value) && Number.isFinite(previous)) change = value - previous;
        if (!Number.isFinite(value)) continue;
        records.push({
          market: row.label || pretty(key), marketKey: canon(row.label || key), tenor: '10Y',
          value, previous, changeBps: Number.isFinite(change) ? change * 100 : null,
    ...
    const walk = (node, path = []) => {
      ...
      if (explicitTenor && TENORS.includes(explicitTenor) && Number.isFinite(v)) {
        ...
        if (mk) {
          ...
          let ch = number(get(node,['change','dailyChange','daily_change']));
          if (!Number.isFinite(ch) && Number.isFinite(prev)) ch = v - prev;
          const rec = { market, marketKey, tenor: explicitTenor, value: v, previous: prev, changeBps: Number.isFinite(ch) ? ch*100 : null,
  ...
  function eligible(r) {
    ...
    return daily && !r.isFallback && Number.isFinite(r.value) && (r.stalenessDays === null || !Number.isFinite(r.stalenessDays) || r.stalenessDays <= MAX_FRESH_DAYS);
  }
  ...
  class BondStatsBase extends HTMLElement {
    ...
    link(){ return this.attr('link',DEFAULT_HOME); }
  ...
  class BondStatsSystemPressure extends BondStatsBase {
    render(feed){
      ...
      this.shadowRoot.innerHTML=this.shell(`
        ...
        <div class="meta"><span><b>${x.markets}</b> fresh markets</span><span>Observation <b>${fmtDate(x.observationDate)}</b></span>${x.topMarket?`<span>Highest <b>${esc(x.topMarket.market)}</b></span>`:''}<span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Open BondStats →</a></span></div>`);
    }
  }
  ...
  class BondStatsSovereignSpread extends BondStatsBase {
    ...
    render(feed){
      ...
      this.shadowRoot.innerHTML=this.shell(`
        ...
        <div class="meta"><span>Observation <b>${fmtDate(x.marketRecord?.date || x.benchmarkRecord?.date)}</b></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Explore markets →</a></span></div>`);
    }
  }
  ...
  class BondStatsFinancialClock extends HTMLElement {
    ...
    render(){
      ...
      const link=this.getAttribute('link')||DEFAULT_HOME;
      ...
      this.shadowRoot.innerHTML=`<style>${baseCss}</style><div class="card"><div class="top"><div><div class="eyebrow">Global Financial Clock</div><div class="title">Live financial-center time</div></div><div class="state"><i class="dot"></i>Live</div></div><div class="grid">${items}</div><div class="meta"><span>IANA time zones · automatic DST</span><span style="margin-left:auto"><a class="link" href="${esc(link)}" target="_blank" rel="noopener">Open full clock →</a></span></div></div>`;
    }
  }
  ...
  function upgradeDataWidgets(root=document){
    root.querySelectorAll('[data-bondstats-widget]').forEach(node=>{
      ...
      [...node.attributes].forEach(a=>{ if(a.name.startsWith('data-') && !['data-bondstats-widget','data-bondstats-upgraded'].includes(a.name)) el.setAttribute(a.name.slice(5),a.value); });
      node.replaceWith(el); el.dataset.bondstatsUpgraded='1';
    });
  }
