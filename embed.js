...
(() => {
  ...
  const LINKS = Object.freeze({
    home: DEFAULT_HOME,
    systemPressure: CONFIG.links?.systemPressure || 'https://botapi33.github.io/bondstats-system-pressure-index/',
    spreadScanner: CONFIG.links?.spreadScanner || 'https://botapi33.github.io/bondstats-curve-spread-scanner/',
    financialClock: CONFIG.links?.financialClock || 'https://botapi33.github.io/bondstats-global-financial-clock/',
    curveScanner: CONFIG.links?.curveScanner || 'https://botapi33.github.io/bondstats-curve-spread-scanner/'
  });
  ...
  class BondStatsSystemPressure extends BondStatsBase {
    ...
    render(feed){
      ...
      this.shadowRoot.innerHTML=this.shell(`
        ...
        <div class="meta"><span><b>${x.markets}</b> fresh markets</span><span>Observation <b>${fmtDate(x.observationDate)}</b></span>${x.topMarket?`<span>Highest <b>${esc(x.topMarket.market)}</b></span>`:''}<span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Open full index →</a></span></div>`);
    }
  }

  class BondStatsSovereignSpread extends BondStatsBase {
    static defaultLink = LINKS.spreadScanner;
    ...
    render(feed){
      ...
      this.shadowRoot.innerHTML=this.shell(`
        ...
        <div class="meta"><span>Observation <b>${fmtDate(x.marketRecord?.date || x.benchmarkRecord?.date)}</b></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Open spread scanner →</a></span></div>`);
    }
  }

  class BondStatsCurveSignal extends BondStatsBase {
    static defaultLink = LINKS.curveScanner;
  ...
  const api = Object.freeze({
    ...
    diagnostics: async()=>{
      ...
      checks.ok = Boolean(checks.customElements && checks.timeZones && checks.feed);
      return checks;
    },
    links: LINKS,
