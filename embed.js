...
(() => {
  ...
  const MAX_REFERENCE_DAYS = 120;
  const MAX_CURVE_DATE_GAP_DAYS = 14;
  const MAX_CHANGE_DATE_GAP_DAYS = 2;
  ...
  function freshnessDays(r) {
    return Number.isFinite(r?.stalenessDays) ? r.stalenessDays : ageDays(r?.date);
  }
  function isDaily(r) {
    const f = String(r?.frequency || '').toLowerCase();
    return f === 'daily' || f === '1d' || f.includes('day');
  }
  function eligible(r) {
    const freshness = freshnessDays(r);
    return isDaily(r) && !r.isFallback && Number.isFinite(r.value) && Number.isFinite(freshness) && freshness <= MAX_FRESH_DAYS;
  }
  function referenceEligible(r) {
    const freshness = freshnessDays(r);
    return !r.isFallback && Number.isFinite(r.value) && Number.isFinite(freshness) && freshness <= MAX_REFERENCE_DAYS;
  }
  function referenceStatus(r) {
    if (!r) return { label:'Unavailable', code:'unavailable', days:null };
    const days = freshnessDays(r);
    if (!Number.isFinite(days)) return { label:'Date unavailable', code:'unknown', days:null };
    if (eligible(r)) return { label:'Fresh daily', code:'fresh', days };
    if (days <= 35) return { label:'Latest available', code:'latest', days };
    if (days <= MAX_REFERENCE_DAYS) return { label:'Delayed', code:'delayed', days };
    return { label:'Unavailable', code:'unavailable', days };
  }
  function latestRecord(feed, market, tenor = '10Y', mode = 'current') {
    const key = canon(market);
    const rule = mode === 'reference' ? referenceEligible : eligible;
    const matches = feed.records.filter(r => rule(r) && r.marketKey === key && r.tenor === tenor);
    return matches.sort((a,b) => {
      const bd=toDate(b.date)?.getTime() || 0, ad=toDate(a.date)?.getTime() || 0;
      if (bd !== ad) return bd-ad;
      const bf=freshnessDays(b), af=freshnessDays(a);
      return (Number.isFinite(af)?af:9999) - (Number.isFinite(bf)?bf:9999);
    })[0] || null;
  }
  function marketRecord(feed, market, tenor = '10Y') {
    return latestRecord(feed, market, tenor, 'current');
  }
  function referenceRecord(feed, market, tenor = '10Y') {
    return latestRecord(feed, market, tenor, 'reference');
  }
  ...
  function comparableDailyChange(a,b){
    const gap=dateGapDays(a,b);
    return Boolean(a && b && eligible(a) && eligible(b) && Number.isFinite(a.changeBps) && Number.isFinite(b.changeBps) && Number.isFinite(gap) && gap <= MAX_CHANGE_DATE_GAP_DAYS);
  }
  ...
  function spread(feed, market, benchmark) {
    const a = referenceRecord(feed, market, '10Y'), b = referenceRecord(feed, benchmark, '10Y');
    if (!a || !b) return { market, benchmark, available:false, spreadBps:null, changeBps:null, changeAvailable:false, marketRecord:a, benchmarkRecord:b, marketStatus:referenceStatus(a), benchmarkStatus:referenceStatus(b) };
    const changeAvailable = comparableDailyChange(a,b);
    return { market:a.market, benchmark:b.market, available:true, spreadBps:(a.value-b.value)*100,
      changeBps:changeAvailable ? a.changeBps-b.changeBps : null, changeAvailable, marketRecord:a, benchmarkRecord:b,
      marketStatus:referenceStatus(a), benchmarkStatus:referenceStatus(b), dateGapDays:dateGapDays(a,b) };
  }

  function curve(feed, market, shortTenor='2Y', longTenor='10Y') {
    const s = referenceRecord(feed, market, shortTenor), l = referenceRecord(feed, market, longTenor);
    if (!s || !l) return { market, shortTenor, longTenor, available:false, label:'Tenor coverage unavailable', short:s, long:l };
    const gap = dateGapDays(s,l);
    if (!Number.isFinite(gap) || gap > MAX_CURVE_DATE_GAP_DAYS) {
      return { market:l.market || pretty(market), shortTenor, longTenor, available:false, label:'Observations too far apart', short:s, long:l, dateGapDays:gap };
    }
    const slopeBps = (l.value-s.value)*100;
    const dynamic = comparableDailyChange(s,l);
    const delta = dynamic ? l.changeBps-s.changeBps : null;
    const avg = dynamic ? (l.changeBps+s.changeBps)/2 : null;
    let label = slopeBps < 0 ? 'Inverted' : slopeBps < 25 ? 'Flat / Positive' : 'Positive Slope';
    if (dynamic && Number.isFinite(delta) && Number.isFinite(avg)) {
    ...
    return { market:l.market, shortTenor, longTenor, available:true, short:s, long:l, slopeBps, slopeChangeBps:delta, label, dynamic, dateGapDays:gap, shortStatus:referenceStatus(s), longStatus:referenceStatus(l) };
  }


  function selfTest() {
    const mk=(market,tenor,value,previous,frequency,stalenessDays,date)=>({
      market,marketKey:canon(market),tenor,value,previous,changeBps:Number.isFinite(previous)?(value-previous)*100:null,
      frequency,stalenessDays,isFallback:false,date,source:'SELFTEST',tier:''
    });
    const feed={meta:{},records:[
      mk('Italy','10Y',4.20,4.16,'daily',0,'2026-08-27'),
      mk('Germany','10Y',3.00,2.99,'monthly',30,'2026-07-28'),
      mk('France','10Y',3.40,3.39,'daily',0,'2026-08-27'),
      mk('United States','10Y',4.50,4.45,'daily',0,'2026-08-27'),
      mk('United States','2Y',4.30,4.30,'daily',0,'2026-08-27'),
      mk('United Kingdom','10Y',5.00,4.98,'daily',0,'2026-08-27'),
      mk('Norway','10Y',4.10,4.11,'daily',0,'2026-08-27'),
      mk('Spain','10Y',3.60,3.59,'daily',0,'2026-08-27'),
      mk('Old Market','10Y',7.00,6.90,'monthly',121,'2026-04-28')
    ]};
    const sp=spread(feed,'Italy','Germany');
    const cv=curve(feed,'United States','2Y','10Y');
    const old=referenceRecord(feed,'Old Market','10Y');
    const pressure=systemPressure(feed);
    const pass = Boolean(
      sp.available && Math.round(sp.spreadBps)===120 && sp.changeAvailable===false && sp.changeBps===null &&
      cv.available && cv.dynamic && Math.round(cv.slopeBps)===20 && cv.label==='Bear Steepening' &&
      old===null && Number.isFinite(pressure.score)
    );
    return { pass, mixedFrequencySpreadBps:sp.spreadBps, mixedFrequencyChangeSuppressed:sp.changeAvailable===false, curveSlopeBps:cv.slopeBps, curveLabel:cv.label, staleReferenceExcluded:old===null, systemPressure:Number.isFinite(pressure.score) };
  }
  ...
  class BondStatsSovereignSpread extends BondStatsBase {
    ...
    render(feed){
      ...
      if(!x.available){
        this.shadowRoot.innerHTML=this.shell(`<div class="top"><div><div class="eyebrow">10Y Sovereign Spread</div><div class="title">${esc(pretty(market))} vs ${esc(pretty(bench))}</div></div><div class="state">Unavailable</div></div><div class="desc">A valid recent 10Y observation is not currently available for both markets. BondStats does not synthesize missing yields or silently use fallback records.</div><div class="meta"><span>Reference window <b>${MAX_REFERENCE_DAYS} days</b></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Explore on BondStats →</a></span></div>`); return;
      }
      const direction=x.changeAvailable?(x.changeBps>0?'Widening':x.changeBps<0?'Compressing':'Unchanged'):'Latest available';
      const ms=x.marketStatus, bs=x.benchmarkStatus;
      this.shadowRoot.innerHTML=this.shell(`
        ...
        <div class="grid"><div class="metric"><div class="k">${x.changeAvailable?'Daily spread change':'Change signal'}</div><div class="v">${x.changeAvailable?fmtBp(x.changeBps):'—'}</div><div class="desc" style="margin-top:5px">${x.changeAvailable?'Comparable fresh daily observations.':'Not shown unless both markets have comparable fresh daily observations.'}</div></div><div class="metric"><div class="k">Market / benchmark</div><div class="v">${fmtYield(x.marketRecord?.value)} / ${fmtYield(x.benchmarkRecord?.value)}</div></div></div>
        <div class="meta"><span>${esc(x.market)} <b>${fmtDate(x.marketRecord?.date)}</b> <i class="badge ${esc(ms.code)}">${esc(ms.label)}</i></span><span>${esc(x.benchmark)} <b>${fmtDate(x.benchmarkRecord?.date)}</b> <i class="badge ${esc(bs.code)}">${esc(bs.label)}</i></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Explore on BondStats →</a></span></div>`);
    }
  }
  ...
  class BondStatsCurveSignal extends BondStatsBase {
    ...
    render(feed){
      ...
      if(!x.available){
        const reason=x.label==='Observations too far apart'?'The requested tenors exist, but their observation dates are too far apart for a reliable curve comparison.':'The current feed does not expose valid recent observations for both requested tenors.';
        this.shadowRoot.innerHTML=this.shell(`<div class="top"><div><div class="eyebrow">Curve Signal</div><div class="title">${esc(pretty(market))} · ${esc(shortT)}/${esc(longT)}</div></div><div class="state">Coverage</div></div><div class="desc">${esc(reason)} BondStats never synthesizes missing curve points or mixes widely separated dates.</div><div class="meta"><span><b>Transparent data rule</b></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Explore on BondStats →</a></span></div>`); return;
      }
      const modeLabel=x.dynamic?'Fresh daily signal':'Latest available curve';
      this.shadowRoot.innerHTML=this.shell(`<div class="top"><div><div class="eyebrow">Curve Signal</div><div class="title">${esc(x.market)} · ${esc(shortT)}/${esc(longT)}</div></div><div class="state"><i class="dot"></i>${esc(x.label)}</div></div><div class="pair" style="margin-top:16px"><div class="big">${Number.isFinite(x.slopeBps)?x.slopeBps.toFixed(0):'—'}</div><div class="unit">bp slope</div></div><div class="grid"><div class="metric"><div class="k">${esc(shortT)}</div><div class="v">${fmtYield(x.short.value)}</div><div class="desc" style="margin-top:5px">${fmtDate(x.short.date)}</div></div><div class="metric"><div class="k">${esc(longT)}</div><div class="v">${fmtYield(x.long.value)}</div><div class="desc" style="margin-top:5px">${fmtDate(x.long.date)}</div></div></div><div class="meta"><span><b>${esc(modeLabel)}</b></span><span>Slope Δ <b>${x.dynamic?fmtBp(x.slopeChangeBps):'—'}</b></span><span style="margin-left:auto"><a class="link" href="${esc(this.link())}" target="_blank" rel="noopener">Explore on BondStats →</a></span></div>`);
    }
  }
  ...
  const api = Object.freeze({
    ...
    getMarket: async(market,tenor='10Y',mode='current')=>latestRecord(await fetchRaw(),market,tenor,mode==='reference'?'reference':'current'),
    selfTest,
    diagnostics: async()=>{
      const logic = selfTest();
      const checks = {
        version: VERSION,
        dataUrl: DATA_URL,
        links: LINKS,
      ...
      try {
        ...
        checks.reference10Y = new Set(feed.records.filter(r=>referenceEligible(r)&&r.tenor==='10Y').map(r=>r.marketKey)).size;
        checks.referencePolicy = `non-fallback · valid value · <=${MAX_REFERENCE_DAYS} days`;
      } catch (e) {
        checks.feed = false;
        checks.feedError = e?.message || String(e);
        checks.systemPressure = false;
      }
      checks.dataPolicy = Boolean((checks.reference10Y || 0) >= (checks.eligible10Y || 0) && (checks.reference10Y || 0) > 0);
      checks.ok = Boolean(checks.customElements && checks.timeZones && checks.publicLinksOnBondStats && checks.feed && checks.systemPressure && checks.logicSelfTest && checks.dataPolicy);
      return checks;
    },
    links: LINKS,
